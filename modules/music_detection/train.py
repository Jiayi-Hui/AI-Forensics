...
from sonics.utils.manifest import materialize_run_snapshot, resolve_dataset_frames
...
def setup_runtime_environment(cfg):
    cuda_count = torch.cuda.device_count() if torch.cuda.is_available() else 0
    cfg.environment.world_size = cuda_count if cuda_count > 0 else 1
    cfg.environment.distributed = cuda_count > 1
    cfg.environment.dist_backend = "nccl" if cfg.environment.distributed else None


def resolve_device(gpu, cfg):
    if torch.cuda.is_available():
        torch.cuda.set_device(gpu)
        print(f"> Using CUDA GPU: {gpu}")
...
def main():
    ...
    # Set up runtime device/distributed training. Apple Silicon MPS is exposed as
    # one logical device, so DDP remains CUDA-only here.
    setup_runtime_environment(cfg)
...
def main_worker(gpu, cfg):
    ...
    device = resolve_device(cfg.environment.gpu, cfg)
    if cfg.environment.mixed_precision and device.type != "cuda":
        print("> Mixed precision is only enabled for CUDA; disabling it for this run")
        cfg.environment.mixed_precision = False

    # Load metadata
    split_frames, dataset_meta = resolve_dataset_frames(cfg)
    ...
    if cfg.environment.gpu == 0:
        _, run_snapshot_path = materialize_run_snapshot(
            cfg=cfg,
            mode="train",
            frames={"train": train_df, "valid": valid_df, "test": test_df},
            output_dir=f"output/{cfg.experiment_name}",
            dataset_meta=dataset_meta,
        )
        print(f"> Saved run snapshot to {run_snapshot_path}")
        ...
        if cfg.environment.gpu == 0:
            ...
            if is_best:
                ...
                best_valid_result = {
                    ...
                    "train_sens": float(train_sens),
                    "train_spec": float(train_spec),
                    "epoch": best_epoch,
                }
                best_metric = current_metric
                best_epoch = epoch + 1
                best_valid_result["epoch"] = best_epoch

                with open(
                    f"output/{cfg.experiment_name}/best_metrics.json",
                    "w",
                    encoding="utf-8",
                ) as fh:
                    json.dump(best_valid_result, fh, indent=2)
